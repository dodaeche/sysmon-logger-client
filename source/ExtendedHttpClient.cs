        ...
        public async void Send(string url, string data)
        {
            try
            {
                using (System.IO.MemoryStream ms = new System.IO.MemoryStream())
                {
                    ...
                    {
                        gzip.Write(Encoding.ASCII.GetBytes(data), 0, data.Length);
                    }
            ...
            catch (Exception ex)
            {
                ...
                {
                    OnError("Error sending SysMon data: " + ex.InnerException.Message);
                }
                else
                {
                    OnError("Error sending SysMon data: " + ex.Message);
                }                
            }
        }
